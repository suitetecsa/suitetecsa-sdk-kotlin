...
class ConnectApi(
    ...
    private fun init() {
        when (val landingResult = connectPortalCommunicator.performAction(CheckConnection()) {
            ...
            is Error -> throw landingResult.throwable
            is Success -> {
                ...
                when (val loginResult = connectPortalCommunicator.performAction(Action.GetPage(action, data)) {
                    ...
                    is Error -> throw loginResult.throwable
                    is Success -> {
    ...
    val userInformation: ResultType<NautaUser>
        get() {
            if (csrf.isBlank()) throw LoginException("Not logged in")
    ...
    private fun loadCsrf(action: Action) {
        when (val csrfResult =
            ...
            is Error -> throw csrfResult.throwable
            is Success -> {
    ...
    fun login(captchaCode: String): ResultType<NautaUser> {
        ...
        return userPortalCommunicator.performAction(action) {
            val user =
                userPortalScraper.parseNautaUser(it.text ?: "", ExceptionHandler.builder(LoginException::class.java).build())
    ...
    fun topUp(rechargeCode: String) {
        ...
        when (val result = userPortalCommunicator.performAction(action) {
    ...
    fun changeEmailPassword(oldPassword: String, newPassword: String) {
        ...
        if (csrf.isBlank()) throw LoginException("Not logged in")
        loadCsrf(preAction)
        val action = ChangePassword(csrf = csrf, oldPassword = oldPassword, newPassword = newPassword, changeMail = true)
        when (val result = userPortalCommunicator.performAction(action) {
            userPortalScraper.parseErrors(it.text ?: "")
        }) {
            is Error -> throw result.throwable
    ...
    fun getConnections(
        ...
        if (connectionsSummary.count != 0) {
            val action = GetActions(connectionsSummary.count, connectionsSummary.yearMonthSelected, internalLarge, reversed, ActionType.Connections)
    ...
    fun getRecharges(
        ...
        if (rechargesSummary.count != 0) {
            val action = GetActions(rechargesSummary.count, rechargesSummary.yearMonthSelected, internalLarge, reversed, ActionType.Connections)
            return userPortalCommunicator.performListAction(action) {
    ...
    fun getTransfers(
        ...
        if (transfersSummary.count != 0) {
            val action = GetActions(transfersSummary.count, transfersSummary.yearMonthSelected, internalLarge, reversed, ActionType.Connections)
            return userPortalCommunicator.performListAction(action) {
    ...
    fun getQuotesPaid(
        ...
        if (quotesPaidSummary.count != 0) {
            val action = GetActions(quotesPaidSummary.count, quotesPaidSummary.yearMonthSelected, internalLarge, reversed, ActionType.Connections)
            return userPortalCommunicator.performListAction(action) {
